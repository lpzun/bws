...
void add_successor(const Thread_State& t, list<Thread_State>& successors) {
	while (true) {
		...
		if (succ != t) { //find if there are some "succ" in successors, return 0 if not
			list<Thread_State>::const_iterator ifind = std::find(successors.begin(), successors.end(), succ);
...
int main(const int argc, const char * const * const argv) {
	try {
		...
		ins.add_argument("s", "number of shared states (i.e. 1 shared variable with range [0..(s-1)])", "", "0");
		ins.add_argument("l", "number of local states", "", "0");
		ins.add_argument("e", "number of thread transitions (no self loops; enter 0 to get info on maximum value)", "",
				"0");
		ins.add_argument("--initial", "the initial thread state", "", "0|0");
		ins.add_argument("--target", "the target state or target state file", "", "0|0");

		ins.add_switch("--adj-list", "whether to print the adjacency list");
		ins.add_switch("--cmd-line", "whether to print the command line");
		ins.add_switch("--bwstree", "whether to print the backward search tree");
		ins.add_switch("--all-paths", "whether to find all of the bws reachable path");
...
//			}
		...
		if (filename == "X") { // make random thread-state transition diagram
			...
			for (ulong i = 1; i < e; ++i) {
				...
				while (true) {
					...
					list<Thread_State>::const_iterator ifind = std::find(successors.begin(), successors.end(), t);
					if (ifind == successors.end()) {
			...
			while (in >> s1 >> l1 >> sep >> s2 >> l2) {
				...
				inverse_adj_list[Thread_State(s2, l2)].push_back(Thread_State(s1, l1));
			}
			in.close();
		}
		...
		BWS bws;
		bool is_reachable = bws.backward_search(init_ts, target, inverse_adj_list);
		cout << "======================================================" << endl;
		cout << " " << target;
		if (is_reachable) {
