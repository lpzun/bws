 ...
 */
bool BWS::is_less_or_eq_init(const Global_State& s,
        const Thread_State& init_ts) {
...
bool BWS::is_less_or_eq(const Global_State& s1, const Global_State& s2) {
    if (s1.shared == s2.shared) {
        for (map<ushort, ushort>::const_iterator iter = s1.locals.begin(), end =
                s1.locals.end(); iter != end; iter++) {
 ...
 */
bool BWS::backward_search(const Thread_State& init_ts,
        const Global_State& target,
    ...
    to_explores_tree.insert(
            std::pair<ushort, Global_State>(curr_depth, bws_start_state));
...
//	cout << "backward search starts from: " << bws_start_state << endl;
    ...
    while (!to_explore_states.empty()) {
        ...
        for (set<Global_State>::const_iterator iter = explored_states.begin(),
                end = explored_states.end(); iter != end; iter++) {
        ...
        if (is_exist_less) {
            ...
            cov_pre_states = compute_cov_predecessors(explore,
                    inverse_adj_list);

            for (set<Global_State>::const_iterator iter =
                    cov_pre_states.begin(), end = cov_pre_states.end();
                    iter != end; iter++) {
                ...
                if (IS_BWS_TREE || IS_ALL) { //insert a new to_explore into output container
                    to_explores_tree.insert(
                            std::pair<ushort, Global_State>(curr_depth, state));
                }
            ...
            for (set<Global_State>::const_iterator iter =
                    explored_states.begin(), end = explored_states.end();
                    iter != end; iter++) {
                if (is_less_or_eq(*iter, explore)) {
            ...
            std::pair<set<Global_State>::iterator, bool> result =
                    explored_states.insert(explore);
            if (!result.second) {
...
set<Global_State> BWS::compute_cov_predecessors(const Global_State& state,
    ...
    for (ushort local = 0; local < Global_State::l; local++) {
        ...
        map<Thread_State, list<Thread_State> >::const_iterator ifind =
                inverse_adj_list.find(curr_ts);
        if (ifind != inverse_adj_list.end()) {
            list<Thread_State> predecessors = ifind->second;
            if (predecessors.size() >= 1) {
                for (list<Thread_State>::const_iterator liter =
                        predecessors.begin(), lend = predecessors.end();
                        liter != lend; liter++) {
                    const Thread_State& pre_ts = *liter;
                    const Global_State& cov_pre = this->update_counter(curr_ts,
                            pre_ts, state);
                    std::pair<set<Global_State>::iterator, bool> p =
                            cov_pre_states.insert(cov_pre);
                    if (!p.second) {
                    }
                }
            }
 ...
 */
Global_State BWS::update_counter(const Thread_State & curr_ts,
        const Thread_State & pre_ts, const Global_State& state) {
    ...
    Global_State cov_pre(pre_ts.shared, ilocals, state.depth + 1, ++state_ID, t,
            state.ID);
    return cov_pre;
}
 ...
 * @param curr_depth
 */
void BWS::output_to_explores_tree(
        const multimap<ushort, Global_State>& output_explores_tree,
        const ushort& curr_depth) {
    cout << endl;
    cout << PPRINT::plural(output_explores_tree.size(), "state")
            << " backward analyzed" << (true ? ":" : "") << endl;
    for (ushort d = 0; d <= curr_depth; ++d) {
        bool depth_found = false;
        for (multimap<ushort, Global_State>::const_iterator cc =
                output_explores_tree.begin(), end = output_explores_tree.end();
                cc != end; ++cc) {
            if (cc->first == d) {
                depth_found = true;
                cout << "(depth = " << std::setw(3) << d << ")" << std::setw(3)
                        << cc->second << endl;
            }
            if (!depth_found) {
 ...
 * @param path
 */
void BWS::output_bws_witness_path(
        const multimap<ushort, Global_State>& output_explores_tree,
    ...
    while (state.ID > 0) {
        cout << PPRINT::widthify(state, 20, PPRINT::LEFTJUST, ' ')
                << std::setw(30) << state.transition << endl;
    ...
    string filename = FILE_NAME_PREFIX + "_path" + COMPARE::num2string(path)
            + ".dot";
 ...
 * @param transitions
 */
void BWS::output_ettd_to_dot_file(const string& filename,
        const list<Transition>& transitions) {
    ofstream fout;
    ...
    if (!fout.is_open()) {
        ...
        fout
                << "// This file is generated from Press, which aims to visualize the Expanded TTD"
                << endl;
        fout << "// NOTE: convert .dot to .ps using the following command:"
                << endl;
        fout << "//\t\t neato -Tps -n2 -s1 <.dot file> -o <.ps file>" << endl;
        fout
                << "// You can also convert to other format, say fig, with corresponding commands"
                << endl;
        fout << endl;

        fout << "digraph ttd {" << endl;
        fout
                << "\t pagedir=BL; nodesep =.5; rankdir=BT; overlap=false; splines = spline;"
                << endl;
        ...
        fout << endl;
        for (list<Transition>::const_iterator itran = transitions.begin(), end =
                transitions.end(); itran != end; itran++) {
            fout << "\t \"(" << itran->src.shared << "," << itran->src.local
                    << ")\" -> \"(" << itran->dest.shared << ","
                    << itran->dest.local << ")\"";
